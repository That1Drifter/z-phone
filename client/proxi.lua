...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread then
        local newAlerts = (Config.PhoneApplications['message'].Alerts - PhoneData.Chats[ChatKey].Unread)
        Config.PhoneApplications['message'].Alerts = newAlerts

        PhoneData.Chats[ChatKey].Unread = 0

        SendNUIMessage({
            action = "RefreshMessageAlerts",
